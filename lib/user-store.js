...
const useMongo = dbType === 'mongodb' || (!dbType && Boolean(process.env.MONGODB_URI));
let mongoClient;
let usersCollection;
...
async function connectMongo() {
  ...
  if (!process.env.MONGODB_URI) {
    if (dbType === 'mongodb') throw new Error('DB_TYPE=mongodb tapi MONGODB_URI belum di-set');
    return null;
  }
  let MongoClient;
  try {
    ...
    if (dbType === 'mongodb') throw new Error('DB_TYPE=mongodb tapi package "mongodb" belum ter-install');
